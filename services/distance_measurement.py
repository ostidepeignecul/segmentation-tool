...
class DistanceMeasurementService:
    ...
    def measure_distance_at_x(
        ...
        try:
            ...
            if class_A not in endview_data:
                return {
                    "x": x,
                    "status": "error",
            ...
            if class_B not in endview_data:
                return {
                    "x": x,
                    "status": "error",
            ...
            if x_index_A is not None:
                ...
                pixels_A = [p for p in endview_data[class_A] if p['x'] == x]
                if self.ENABLE_PERF_LOGS:
            ...
            if x_index_B is not None:
                ...
                pixels_B = [p for p in endview_data[class_B] if p['x'] == x]
                if self.ENABLE_PERF_LOGS:
    ...
    def measure_distance_all_x(
        ...
        try:
            ...
            if class_A in endview_data:
                x_index_A = self._build_x_index(endview_data[class_A])
                x_positions_A = set(x_index_A.keys())
            
            if class_B in endview_data:
                x_index_B = self._build_x_index(endview_data[class_B])
