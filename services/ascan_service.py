...
class AScanService:
    ...
    def build_profile(
        ...
        slice_idx = self._clamp(model.current_slice or 0, num_slices)
        ...
        axis_map = model.get_axis_map()
        ultrasound_axis = axis_map.get("ultrasound", 2)
        ...
        positions = self._axis_positions_for_profile(model, normalized.size)
    ...
    def map_profile_index_to_point(
        ...
        if ultrasound_axis == 0:
            slice_idx = self._clamp(profile_idx, volume.shape[0])
            model.set_current_slice(slice_idx)
            x = current_point[0] if current_point else width // 2
            y = current_point[1] if current_point else height // 2
            return x, y
        return None
    ...
    def _axis_positions_for_profile(
        ...
        positions = model.get_axis_positions("ultrasound")
